namespace GraphIABook.Chains.Chapter25;

using Microsoft.SemanticKernel;
using System.Collections.Generic;

/// <summary>
/// Chapter 25 — SK chain baseline (heuristic) for route selection.
/// Uses a simple moving-average (historical mean) to choose a route and then
/// simulates execution latency for the chosen route. Mirrors the graph version
/// that uses a GNN-like predictor to inform routing.
/// </summary>
public static class ChainChapter25
{
	public static async Task<string> RunAsync(string input)
	{
		...
		var build = KernelFunctionFactory.CreateFromMethod(
			(KernelArguments a) =>
			{
				...
				// Chosen so that baseline picks B (hist mean best), while the GNN-like
				// predictor (graph) will prefer A due to structural features.
		...
		var execute = KernelFunctionFactory.CreateFromMethod(
			async (KernelArguments a) =>
			{
				...
				return $"answer(chain25): route={route}; expectedMs={expected}; actualMs={t}";
			},
			"ExecuteRoute");
