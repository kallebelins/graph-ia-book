namespace GraphIABook.Graphs.Chapter25;
...
/// <summary>
/// Chapter 25 — SKG graph with a GNN-like predictor informing route selection.
...
/// </summary>
public static class GraphChapter25
{
	...
	public static GraphExecutor CreateExecutor()
	{
		...
		// GNN-like predictor: predicts latency score = w1*degree + w2*betweenness*1000 (ms proxy)
		var predict = new FunctionGraphNode(KernelFunctionFactory.CreateFromMethod((KernelArguments a) =>
		{
			...
			// Weights pick lower degree and lower betweenness as better (lower cost)
		...
		var execute = new FunctionGraphNode(KernelFunctionFactory.CreateFromMethod(async (KernelArguments a) =>
		{
			...
			return $"answer(graph25): route={route}; actualMs={t}";
		...
		var exec = new GraphExecutor("ch25_gnn_routing", "GNN-like predictor informs orchestration routing");
